...
END = pd.Timestamp('2000-09-07')  # day before the example crop resets/harvests
OUT = Path('phase3/run')
CACHE = Path('phase3/herbie_cache')
...
# Fetch only dates that could plausibly be reached after a baseline decision.
# We include five days after each baseline irrigation event even though the
# decision rule can defer for only two days. This buffer allows subsequent
# irrigation trigger dates to shift when an earlier event is delayed/skipped.
events = pd.read_csv(OUT / 'baseline_irrigation_events.csv')
...
# Warm up Herbie config/cache before concurrent calls.
_ = Herbie(
...
rows = []
# Independent issue dates are fetched concurrently to keep the one-season
# proof-of-concept reasonably fast on GitHub-hosted runners.
...
forecast.to_csv(OUT / 'gefs_season_decision_forecasts.csv', index=False)
...
meta = {
    ...
    'candidate_selection': 'baseline irrigation dates plus 0-5 day buffer, clamped to decision season',
    'note': 'Proof-of-concept. GEFS 00 UTC issue date is mapped to the APSIM local decision date without sub-daily timezone correction.',
}
(OUT / 'gefs_season_metadata.json').write_text(json.dumps(meta, indent=2))
print('\nCompleted one-season forecast retrieval')
